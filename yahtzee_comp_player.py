  ...
  for i in xrange(kYahtzeeScoredIndex):
    if state[i] == 0:
      if (state[kDiceIndex:].count(state[kDiceIndex]) == 5 and
          ...
          (yahtzee.all_score_types[i] == 'Full House' or
           yahtzee.all_score_types[i] == 'Small Straight' or
           yahtzee.all_score_types[i] == 'Large Straight')):
        # Illegal to use wildcard in this case
        continue
      actions.append(yahtzee.RecordScore(yahtzee.all_score_types[i]))
  if state[kRollsIndex] < 2:
    for roll in rolls:
      actions.append((yahtzee.Roll(roll)))
...
def reward_func(state, action):
  ...
  if isinstance(action, yahtzee.Roll):
    ...
    sheet = yahtzee.ScoreSheet(state[:kYahtzeeScoredIndex],
...
def trans_func(state, action):
  ...
  if isinstance(action, yahtzee.Roll):
    ...
    state_list[yahtzee.all_score_types.index(action.score())] = 1
